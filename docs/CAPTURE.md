...
- Repeat area capture (`repeatArea`, default ⌃⌘⇧4): re-captures the last manually selected area rect instantly, without re-entering selection mode. Every completed manual area selection (live or frozen) is persisted via `ScreenshotLastAreaStore` (UserDefaults `screenshot.lastAreaRect`, AppKit global screen points — same storage pattern as `recording.lastAreaRect`). `ScreenCaptureViewModel.captureRepeatArea()` validates the stored rect against the currently connected displays (notifying via native notification with in-app toast fallback when missing or off-screen — repeat never re-enters the selection flow), then calls `ScreenCaptureManager.captureArea(rect:)` directly, so post-capture routing (Quick Access, clipboard, annotate, history) is identical to a fresh selection. Pressing **Return** during an area-screenshot selection also repeats the stored area (per-session opt-in `allowsRepeatAreaCompletion` on `AreaSelectionController.startSelection`, so OCR/cutout selections sharing `.screenshot` mode keep their default behavior), and `snapzy://capture/repeat-area` exposes the same action to automation.
...
- The live overlay is a `.nonactivatingPanel` and never becomes key (activating would dim live windows), so AppKit's cursor-rect machinery does not self-heal the crosshair after an external reset. The crosshair, key focus, and overlay windows ordering are kept sticky by re-asserting them via `NSWorkspace.activeSpaceDidChangeNotification` and `NSApplication.didBecomeActiveNotification` observers during a session (`handleSessionSpaceOrActivationChange`): they restore key focus on the keyboard-owner panel, order the overlays to the front, refresh cursors, force a layer recommit (`needsDisplay`), and re-arm the presentation watchdog (see "Selection overlay architecture"). Overlay pointer events (hover/drag) also request this recovery via `areaSelectionWindowDidRequestDisplayActivation`, coalesced to a low-rate safety net (`eventDrivenSessionRecoveryMinInterval`, 0.5s) — running it per pointer tick cost multiple ms of main-thread time per mouse event (watchdog WindowServer window-list query, per-window orderFront/cursor-rect/needsDisplay passes, log writes) and visibly starved the crosshair/coordinate updates that share the thread. The app itself is deliberately never reactivated — area selection is the one overlay flow that stays non-activating so foreground-window capture still observes the previously frontmost app.
...
- `AreaSelectionController` (`Snapzy/Services/Capture/AreaSelectionWindow.swift`, `@MainActor` singleton) orchestrates sessions: pooled per-display `AreaSelectionWindow` panels (`prepareWindowPool()` pre-allocates at launch for <150ms activation; pool refreshed on screen-parameter changes — mid-session, newly attached displays get configured and shown immediately so no hidden click fall-through hole appears; pool **rebuilt from scratch** on Space-churn signals — `NSWorkspace.activeSpaceDidChangeNotification` and `didWakeNotification`, debounced 500ms, idle only — because a long-lived ordered-out panel's WindowServer all-spaces membership rots as Spaces churn beneath it, and a fresh panel is the only reliable membership reset), Escape monitors (local + global), `withDisplayOverlayHidden(Async)` for snapshot grabs while the overlay is visible, and Space/app-activation observers that trigger backdrop recapture.
- Session lifecycle is single-fire and re-entrancy safe. Starting a session while one is presenting tears the previous one down through the normal cancel path first — its completion is invoked once with `nil` (so each feature's own cancel cleanup runs: `ScreenCaptureViewModel.isAreaSelectionActive` resets, hidden windows restore, frozen sessions invalidate), its observers and Quick Access suspension are released, and the new session then starts clean. Completions are snapshotted and cleared before invocation, so a completion that synchronously calls back into the controller (live area mode calls `cancelSelection()` to dismiss) cannot fire twice.
- A bounded **presentation watchdog** (`scheduleSessionPresentationWatchdog`) verifies that pooled panels actually present, not just report `isVisible`: it checks once on the next run-loop turn after `activatePooledWindows`, then every 0.5s for the first ~3s of the session, and re-arms for the same window after every space/app-activation change (space switches are the prime suspect for a panel losing its all-spaces membership — the field failure where the overlay shows on one desktop Space but not another while live-passthrough selection keeps working invisibly). Each check evaluates `AreaSelectionPresentationLogic` — visibility, active-space membership, frame-vs-screen sync, alpha, occlusion, and the WindowServer's own on-screen list (`CGWindowList` `.optionOnScreenOnly` matched by window number, the ground truth AppKit properties can all miss for a `.canJoinAllSpaces` panel; a failed query is "unknown", never an anomaly). Heals escalate per display via `AreaSelectionPresentationLogic.healAction`: membership loss (`offActiveSpace` / `notOnScreenPerWindowServer`) goes straight to **pooled-window recreation** (`recreatePooledWindow` — a fresh panel's new WindowServer record re-establishes all-spaces membership at order-in time; the earlier order-out + `collectionBehavior`-toggle re-assert was field-proven to never repair membership loss and was removed), transient anomalies (occlusion turbulence, refused ordering, stale frame) get one cheap `orderFrontRegardless` + frame-resync re-assert before escalating to recreation on the next tick. Recreation is capped at 2 per display per session, defers while a non-passthrough drag is in progress, and has a single last-resort rung that activates the app before recreating once more (the inline-annotate formula) when the cap is spent. Anomalies are logged with the chosen heal action and the overlay's layer diagnostics (`AreaSelectionOverlayView.presentationDiagnostics()`), and a later clean check logs the recovery — so field reports from the diagnostic bundle show the failing state, the heal applied, and whether it worked.
...
- Recognition is routed by `OCRModelResolver` (from the persisted selection, `ocr.selectedModel`) to an `OCRProvider` for the active provider: `VisionOCRProvider` (built-in Apple Vision, default) or `RemoteOCRProvider` (custom OpenAI-compatible chat-completions endpoint, base64 JPEG, 60s timeout). `OCREngine` is `vision` / `remote`.
...
- OCR providers are managed under Settings → Capture → OCR → OCR Model (see [`PREFERENCES.md`](PREFERENCES.md)). Apple Vision is always available. Custom endpoint metadata is JSON-persisted in `ocr.customModels`; optional API keys live in the macOS Keychain (service `com.trongduong.snapzy.ocr`) and are never exported.
- Only one provider is active. A removed or unavailable custom endpoint resolves to built-in Vision and is persisted back by `OCRModelResolver`. Legacy `dl:<id>` selections from older configurations are also canonicalized to `builtin`.
...
| `Snapzy/Services/Media/OCR/` | `OCRProvider` engines (built-in Vision and remote OpenAI-compatible), custom endpoint persistence, OCR profiles and result normalization |
